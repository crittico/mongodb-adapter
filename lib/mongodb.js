...
MongoDB.prototype.exp = function exp(model, cb, params) {
    if (params.type === "csv") {
        ...
        for (var f in this._models[model].properties) {
            if (f !== 'id' && f !== 'd')
                /*if (this._models[model].properties[f] instanceof Object)
                    for (var f2 in this._models[model].properties[f])
                        fields += f + "." + f2 + ",";
                else*/
                fields += f + ",";
            if (f === 'd') {
                fields += "d.s,d.l,d.p,d.d,";
            }
        }       
        ...
        " -q '" + whereString + "' -f " + fields + " -o " + params.filename + ";";
        cb(null, cmd);
    }
