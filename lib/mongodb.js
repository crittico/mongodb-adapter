...
MongoDB.prototype.all = function all(model, filter, callback) {
    ...
    if (!filter.hint) 
      cursor = this.collection(model).find(query, filter.project);
    else 
      cursor = this.collection(model).find(query, filter.project).hint(filter.hint);
...
MongoDB.prototype.mapReduce = function aggregate(model, cb, map, reduce, options) {
    this.collection(model).mapReduce(map, reduce, options, function (err, object) {
      cb(err, object);
    });
};
